...
const ScalingProgress = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (isTrader) {
        ...
        const response = await axios.get('/api/v1/scaling/my-progress', config);
        setScalingData(response.data.data);
      } else if (isAdmin || isAffiliate) {
        // Admin/Affiliate: Get all users (filtered by backend)
        const response = await axios.get('/api/v1/scaling/admin/all-users', config);
  ...
  const handleScaleUp = async () => {
    try {
      ...
      await axios.post('/api/v1/scaling/scale-up', {}, config);
      alert('Successfully scaled up to next tier!');
      fetchData();
    } catch (err) {
      alert(err.response?.data?.error || 'Failed to scale up');
    }
  };
