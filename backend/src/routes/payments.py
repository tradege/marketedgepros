...
def get_payments(current_user):
    ...
    try:
        ...
        payments_query = Payment.query.filter_by(user_id=current_user.id).order_by(Payment.created_at.desc())
...
def create_payment_intent(current_user):
    ...
    try:
        ...
        payment_data = PaymentService.create_payment_intent(challenge, current_user)
...
@token_required
def refund_payment(current_user, challenge_id):
    """Request refund for a challenge"""
    try:
        challenge = Challenge.query.get_or_404(challenge_id)
        
        # Verify ownership or admin
        if challenge.user_id != current_user.id and current_user.role != 'admin':
            return jsonify({'error': 'Access denied'}), 403
...
@token_required
def get_payment_status(current_user, payment_intent_id):
    """Get payment status"""
    try:
