...
def create_app(config_name=None):
    ...
    # Rate Limiting - Always initialize limiter (required by routes)
    from src import limiter as global_limiter
    global_limiter.init_app(app)
    
    # Only enable storage-backed rate limiting if configured
    if app.config.get('RATELIMIT_ENABLED'):
        app.config['RATELIMIT_STORAGE_URL'] = app.config.get('RATELIMIT_STORAGE_URL')
        app.config['RATELIMIT_DEFAULT'] = "5000 per day;1000 per hour"
    
    # Initialize Talisman for security headers (disabled in testing)
    import os
    ...
    from src.logging_config import setup_logging, log_request_start, log_request_end
    setup_logging(app)
